    ...
    return NextResponse.json(
      {
        ...
        setup: "Add XAI_API_KEY to .env.local to enable live X summaries.",
      },
      { status: 503 },
    );
  }
...
function buildPrompt(from: Date, to: Date): string {
  return `Search X posts from ${from.toISOString()} to ${to.toISOString()} for market-relevant discussion about AI and agent companies.

Focus on frontier models, OpenAI, Anthropic, Google Gemini, xAI/Grok, Meta AI, NVIDIA AI demand, Cursor/Anysphere, coding agents, enterprise agents, model pricing, benchmark releases, safety/regulation, and AI infrastructure.

Write a concise dashboard update for an developer and investor audience:
- 4 short bullets under "Top X narratives"
- 2 short bullets under "What changed"
- 2 short bullets under "What to watch"

Rules:
- Use only information supported by X search results.
- Prefer recent primary-source posts, company accounts, researchers, builders, investors, and credible reporters.
